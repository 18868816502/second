...
public class BankCardTextWatcher implements TextWatcher {
    ...
    public static void watch(EditText editText, int maxLength) {
    ...
    public void afterTextChanged(Editable s) {
        if (isChanged) {
            ...
            //selection index
            if (totalSpace > space) selectionIndex += (totalSpace - space);

            char[] tempChar = new char[buffer.length()];
            buffer.getChars(0, buffer.length(), tempChar, 0);
            String str = buffer.toString();
            if (selectionIndex > str.length()) selectionIndex = str.length();
            else if (selectionIndex < 0) selectionIndex = 0;

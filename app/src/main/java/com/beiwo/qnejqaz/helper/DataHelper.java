...
public class DataHelper {
    ...
    public static final String EVENT_VIEWID_TOOLHOMEPAGE = "ToolHomePage";          //工具页面
    public static final String EVENT_VIEWID_TALLYHOMEPAGE = "TallyHomePage";        //记账首页
    public static final String EVENT_VIEWID_LOANBILLPAGE = "LoanBillPage";          //贷款账单页（网贷）
    ...
    //24小时
    public static final int MAX_SECOND = 24 * 60 * 60 * 1000;

    Api api;
    Context context;

    private DataHelper(Context mContext) {
        api = Api.getInstance();
        context = mContext;
    }
    ...
    public static DataHelper getInstance(Context mContext) {
        if (sInstance == null) {
            synchronized (DataHelper.class) {
                if (sInstance == null) {
                    sInstance = new DataHelper(mContext.getApplicationContext());
                }
            }
        }
        return sInstance;
    ...
    public void event(final String type, String viewId, String eventId, long duration) {
        ...
        map.put("type", type);//事件类型
        map.put("viewId", viewId);//页面Id (事件类型 = 启动应用 可以为空)

...
public class BannerLayout extends RelativeLayout {
    ...
    private void init(AttributeSet attrs, int defStyle) {
        ...
        boolean indicatorSelectedStroke = array.getBoolean(R.styleable.BannerLayoutStyle_indicator_selected_stroke, false);
        boolean indicatorUnSelectedStroke = array.getBoolean(R.styleable.BannerLayoutStyle_indicator_unselected_stroke, false);
        array.recycle();
        ...
        if (indicatorUnSelectedStroke) {
            unSelectedGradientDrawable.setStroke(dp2px(1.1f), selectedIndicatorColor);
        } else {
        ...
        if (indicatorSelectedStroke) {
            selectedGradientDrawable.setStroke(dp2px(1.1f), selectedIndicatorColor);
        } else {
    ...
    private ImageView getImageView(String url, final int position) {
        ...
        if (imageLoader != null) imageLoader.displayImage(getContext(), url, imageView);
        return imageView;
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (Math.abs(currentX - downX) > Math.abs(currentY - downY) && Math.abs(currentX - downX) > mTouchSlop) {
    ...
    private void startAutoPlay() {
        stopAutoPlay(); // 避免重复消息
        if (isAutoPlay) handler.sendEmptyMessageDelayed(WHAT_AUTO_PLAY, autoPlayDuration);
    }

    @Override
    protected void onWindowVisibilityChanged(int visibility) {
        super.onWindowVisibilityChanged(visibility);
        if (visibility == VISIBLE) startAutoPlay();
        else stopAutoPlay();
    }
    ...
    private class LoopPagerAdapter extends PagerAdapter {
        ...
        @Override
        public int getCount() {
            return Integer.MAX_VALUE;//Integer.MAX_VALUE = 2147483647
        }
        ...
        public Object instantiateItem(ViewGroup container, int position) {
            if (views.size() > 0) {
                ...
                if (container.equals(view.getParent())) container.removeView(view);

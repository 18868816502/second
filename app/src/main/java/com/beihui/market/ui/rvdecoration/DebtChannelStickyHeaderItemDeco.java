...
public abstract class DebtChannelStickyHeaderItemDeco extends RecyclerView.ItemDecoration {
    ...
    public DebtChannelStickyHeaderItemDeco(Context context) {
    ...
    public void getItemOffsets(Rect outRect, View itemView, RecyclerView parent, RecyclerView.State state) {
        ...
        int pos = parent.getChildAdapterPosition(itemView) - 1;
        if (pos >= 0) {
    ...
    public void onDrawOver(Canvas canvas, RecyclerView recyclerView, RecyclerView.State state) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            //添加了头部，item的范围应该是[1, adapter'itemCount - 1),需要 -1
            int pos = recyclerView.getChildAdapterPosition(childView) - 1;
            if (pos >= 0) {
                String curHeaderName = getHeaderName(pos);
                if (i == 0) {
